...
(function($) {
	...
	var externalAddon = {
		...
		events: function() {
			...
			$(document).on('click', '.wfactory-483-process-button', function() {
				var $this = $(this),
				...
				var action = ('creativemotion' === storage)
				             ? 'wfactory-483-creativemotion-install-plugin'
				             : 'install-plugin';
				...
				if( storage === 'freemius' || ((storage === 'wordpress' || storage === 'creativemotion' || storage === 'internal') && (plugin_action === 'activate' || plugin_action === 'deactivate')) ) {
					action = 'wfactory-483-intall-component';
				...
				$.wfactory_483.hooks.run('core/components/pre_update', [$this, data]);

				self.sendRequest(data, function(response) {
					if( !response || !response.success ) {
						$.wfactory_483.hooks.run('core/components/update_error', [
							$this,
							data,
							response
						]);
					...
					if( response.success ) {
						...
						if( plugin_action === 'install' ) {
							...
							$.wfactory_483.hooks.run('core/components/installed', [
								$this,
								data,
								response
							]);
							...
							$.wfactory_483.hooks.run('core/components/pre_activate', [
								$this,
								data,
								response
							]);
							...
							$.wfactory_483.hooks.run('core/components/deactivated', [
								$this,
								data,
								response
							]);
							...
							$.wfactory_483.hooks.run('core/components/deleted', [$this, data, response]);
						}
					} else {
						if( plugin_action === 'install' ) {
					...
					$.wfactory_483.hooks.run('core/components/updated', [$this, data, response]);
				});
		...
		sendRequestToComponentActivationPrepare: function(componentButton, sendData, button_i18n) {
			...
			sendData.action = 'wfactory-483-prepare-component';
			...
			this.sendRequest(sendData, function(response) {
				...
				if( !response || !response.success ) {
					...
					$.wfactory_483.hooks.run('core/components/activation_error', [
						componentButton,
						sendData,
						response
					]);
				...
				$.wfactory_483.hooks.run('core/components/activated', [
					componentButton,
					sendData,
					response
				]);
		...
		sendRequest: function(data, callback) {
			...
			$.ajax(ajaxurl, {
				...
				error: function(xhr, ajaxOptions, thrownError) {
					...
					$.wfactory_483.hooks.run('core/components/ajax_error', [
						xhr,
						ajaxOptions,
						thrownError
					]);
